...
struct RunningWorkout {
    ...
    func averageMile() -> Double {
        return (distance / time)

    }
}

var myRun = RunningWorkout(distance: 1600, time: 0.25, elevation: 2000)

print(myRun.averageMile())
...
struct Steps {
    var steps: Int {
        ...
        willSet {
            if newValue == goal {
                print("Congratulations! You met you're goal!")
            }
        }
        didSet {
            if oldValue >= goal {
                print("you already hit your goal")
            }
        }
    }
    var goal: Int
...
var stepStatus = Steps.init(steps: 5, goal: 10)
