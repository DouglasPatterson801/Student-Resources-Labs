...
func checkInventory(item: String) -> Double? {
    if let stock = stock[item] {
        if stock > 0 {
            print("We have \(item) available for \(prices[item]!) each.")
            return prices[item]
        } else {
            print("I'm sorry, we are all out of \(item)")
            return nil
        }
    } else {
        return nil
...
checkInventory(item: "Chips")
checkInventory(item: "Donuts")
